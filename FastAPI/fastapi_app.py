# main.py — FastAPI serving layer for pricing recommendations + KPIs (hardened)
import os
import io
import numpy as np
import pandas as pd
...
MODEL_PATH = os.getenv("PRICING_MODEL_PATH", "GradientBoosting_opt_price_model.joblib")
...
# ===== Load artifacts =====
try:
    # This loads a historical/baseline dataset for KPI comparison against a single-row recommendation
    df_base_all = pd.read_csv(DATA_CSV)
    logger.info("Loaded baseline CSV rows=%d", len(df_base_all))
except Exception as e:
    # Ensure this file (dynamic_pricing.csv) is available in the server environment
    raise RuntimeError(f"Could not load {DATA_CSV}: {e}")

try:
    # This loads the trained ML model pipeline
    best_pipe = joblib.load(MODEL_PATH)
    logger.info("Loaded model from %s", MODEL_PATH)
except Exception as e:
    # Ensure the model file (pricing_completion_model.joblib) is available
    raise RuntimeError(f"Could not load model {MODEL_PATH}: {e}")

# ====== Shared engineering (MUST match your notebook logic) ======
# NOTE: The feature engineering logic in this function is derived from the ML notebook and is
# crucial for both training and serving to work.
def _engineer(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "Customer_Loyalty_Status" in df.columns:
        _loy_map = {"Regular": 0, "Silver": 1, "Gold": 2} # NOTE: The notebook used Bronze/Platinum, adjust this map if needed to match aipriceoptima.py's definition.
        df["Loyalty_Score"] = (
    ...
    if "Vehicle_Type" in df.columns:
        _veh_map = {"Economy": 1.0, "Premium": 1.25} # NOTE: The notebook used 1.0/1.5, adjust this map if needed to match aipriceoptima.py's definition.
